...
printf '%s\n' "$SYMS" | grep -E 'JNI_OnLoad|taylor_android_|nativeOnOrientationChange|Device|Vector3' || true
printf '%s\n' "$SYMS" | grep -E 'JNI_OnLoad' >/dev/null
printf '%s\n' "$SYMS" | grep -E 'nativeOnOrientationChange' >/dev/null
printf '%s\n' "$SYMS" | grep -E 'taylor_android_get_accelerometer' >/dev/null
printf '%s\n' "$SYMS" | grep -E 'taylor_device_drain_callbacks' >/dev/null
